...
InputBuffer = dict[str, Tensor | None]
IncrementalState = dict[str, InputBuffer]
...
class MultiheadAttention(nn.Module):
    ...
    def reorder_incremental_state(
        self, incremental_state: IncrementalState, new_order: Tensor
    ):
    ...
    def _get_input_buffer(
        self, incremental_state: IncrementalState | None
    ) -> InputBuffer:
        result = self.get_incremental_state(incremental_state, "attn_state")
        if result is not None:
            return result
        else:
            empty_result: InputBuffer = {}
            return empty_result
